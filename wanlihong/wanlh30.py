...
from Ui_wanlihong31 import Ui_MainWindow  # 导入由 pyuic5 编译生成的 UI 类
...
class DocumentGenerationTool(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.setupUi(self)  # 确保 UI 组件被正确初始化
    ...
    def initialize_list(self):
        """初始化顺序列表"""
        if hasattr(self, 'listwidget_order'):
            self.listwidget_order.addItems(self.sections)
        else:
            QMessageBox.critical(self, "Error", "ListWidget 'listwidget_order' not found in UI.")
