...
from Ui_wanlihong21 import Ui_MainWindow  # 导入编译后的 UI 文件类
...
class ImageStitchingTool(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.button_switch_position.clicked.connect(self.switch_position)
    ...
    def stitch_images(self):
        ...
        if self.crop1 is not None and self.crop2 is not None:
            stitched_image = self.combine_images(self.crop1, self.crop2)
    ...
    def switch_position(self):
        ...
        QMessageBox.information(self, "Switch Position", "Images switched successfully!")
        self.stitch_images()

    def combine_images(self, img1, img2):
        """根据切换状态拼接图像"""
        if self.switched:
            return np.hstack((img2, img1))
        else:
